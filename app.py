...
# Define a route for the 'file_processed.html' page, to which the app ...
# ...navigates after user submits a file for processing
...
def process_file():
    ...
    if request.method == "POST":
        ...
        # Check if the file type is Excel XLSX and read the first ...
        # ...sheet into a DataFrame
        if input_file.filename.endswith(".xlsx"):
            file_type = "xlsx"
        ...
        if not any("address" in col.lower() for col in cols):
            # If so, render and return the 'invalid_file_uploaded.html' ...
            # ...template
        ...
        for col in cols:
            ...
            if "address" in col.lower():
                # Define the names for latitude column, longitude column, ...
                # ...and map column
                lat_col = col + "_latitude"
                lon_col = col + "_longitude"
                map_col = col + "_map"
                # Add two new columns to the DataFrame for geocoding outputs
                # Apply geocoding to each address in the dataframe to ...
                # ...get the latitude
                df[lat_col] = df[col].apply(
                    lambda x: (
                        ArcGIS().geocode(x).latitude
                        if ArcGIS().geocode(x) is not None
                        else None
                    )
                )
                # Apply geocoding to each address in the dataframe to get ...
                # ...the longitude
                df[lon_col] = df[col].apply(
                    lambda x: (
                        ArcGIS().geocode(x).longitude
                        if ArcGIS().geocode(x) is not None
                        else None
                    )
                )
                ...
                # Create a Folium feature group for collecting markers ...
                # ...for all addresses
                ...
                for row_index in range(len(df)):
                    ...
                    if (not math.isnan(lat)) and (not math.isnan(lon)):
                        # Create a default blue marker for the current...
                        # ...coordinate
                ...
                # Add the feature group carrying all markers to the ...
                # ...Folium map
                full_map.add_child(fg)

                # Create an empty list for collecting all the map HTML ...
                # ...templates
                list_map_url = []
                # Iterate over each row in the DataFrame
                for row_index in range(len(df)):
                    ...
                    # Check if latitude and longitude values are valid
                    if (not math.isnan(lat)) and (not math.isnan(lon)):
                        # Create a featured red marker to highlight the ...
                        # ...current coordinate
                        ...
                        # Set the center of the map view to the current ...
                        # ...coordinate
                        full_map.location = [lat, lon]
                        # Define a file name for the map HTML template
                        map_url = "map_{}_lat={}_lon={}.html".format(
                            col.replace(" ", "_"), lat, lon
                        )
                        ...
                        # Place the default blue marker to overlay the red ...
                        # ...featured marker
                        ...
                        # If the current latitude or longitude values ...
                        # ...are NaN, add None to the list
        ...
        # Replace the table data cells containing map URLs with ...
        # ...clickable links
        data_table_html = re.sub(
            r"<td>(map_\S+_lat=[-\d\.]+_lon=[-\d\.]+\.html)</td>",
        ...
        # Save the DataFrame to either CSV or Excel file based on the ...
        # ...input file type
        if file_type == "csv":
        ...
        # Render and return the 'file_processed.html' template that is ...
        # ...formatted with the output file name and HTML of the DataFrame
